...
const childProgramIndexThreshold = 8;

/**
 * Marks stack values that should be emitted literally instead of wrapped in source nodes.
 */
class Literal {
  /**
...
class JavaScriptCompiler {
  ...
  appendToBuffer(source, location, explicit) {
    ...
    if (this.environment.isSimple && !this.forceBuffer) {
  ...
  compile(environment, options, context, asObject) {
    ...
    this.forceBuffer = !!environment.forceBuffer;
    ...
    this.stackSlot = 0;
    this.stackVars = [];
    this.aliases = createNewLookupObject();
    this.registers = createNewLookupObject();
    this.registers.list = [];
    this.hashes = [];
    this.compileStack = [];
    this.inlineStack = [];
  ...
  mergeSource(varDeclarations) {
    let isSimple = this.environment.isSimple,
      ...
      bufferStart,
      bufferEnd;
    const sourceLines = this.source.source;
  ...
  pushStringParam(string, type, idType, idName, idChild) {
    this.pushContext();
    this.pushString(type);
    if (idType != null) {
      this.pushId(idType, idName, idChild);
    }
  ...
  pushHash() {
    ...
    this.hash = {
      values: createNewLookupObject(),
      types: createNewLookupObject(),
      contexts: createNewLookupObject(),
      ids: createNewLookupObject()
    };
  }
  ...
  compileChildren(environment, options) {
    ...
    for (let i = 0, l = children.length; i < l; i++) {
      ...
      if (existing == null) {
        const index = this.context.programs.push('') - 1;
        child.index = index;
        ...
        this.indexCompiledProgram(child, index);
  ...
  matchExistingProgram(child) {
    if (this.context.environments.length < childProgramIndexThreshold) {
      return this.matchExistingProgramByScan(child);
    }

    const signature = this.getCompiledProgramSignature(child);
    if (signature === undefined) {
      return this.matchExistingProgramByScan(child);
    }

    const indexes = this.getProgramSignatureIndex().get(signature);
    if (!indexes) {
      return undefined;
    }

    for (let i = 0, len = indexes.length; i < len; i++) {
      let environment = this.context.environments[indexes[i]];
   ...
   * @returns {void}
   */
  indexCompiledProgram(child, index) {
    if (!this.context.environmentIndexesBySignature) {
      return;
    }
    ...
    let indexes = this.context.environmentIndexesBySignature.get(signature);
    if (!indexes) {
      indexes = [];
      this.context.environmentIndexesBySignature.set(signature, indexes);
    }
    indexes.push(index);
  }
   ...
   */
  getProgramSignatureIndex() {
    let index = this.context.environmentIndexesBySignature;
    if (index) {
      return index;
    }
    ...
    for (let i = 0, len = environments.length; i < len; i++) {
      ...
      const signature = this.getCompiledProgramSignature(environment);
      if (signature === undefined) {
        continue;
      }

      let indexes = index.get(signature);
      if (!indexes) {
        indexes = [];
        index.set(signature, indexes);
      }
      indexes.push(i);
    }

    this.context.environmentIndexesBySignature = index;
    return index;
  }
  ...
  getCompiledProgramSignature(environment) {
    if (typeof environment.getSignature === 'function') {
      return environment.getSignature();
    }
    return environment.signature;
  }
  ...
  setupParams(helper, paramSize, params) {
    let options = createNewLookupObject(),
      contexts = [],
      types = [],
      ids = [],
      objectArgs = !params,
    ...
    if (this.options.data) {
      options.data = 'data';
      options.partialName = 'data && data.partialName';
    }
    if (this.useBlockParams) {
  ...
  setupHelperArgs(helper, paramSize, params, useRegister) {
    ...
    options.loc = serializeLocation(this.source.currentLocation);
    options = this.objectLiteral(options);
    if (useRegister) {
...
/**
 * Resolves a strict path lookup into emitted JavaScript source.
 *
 * @param {boolean} requireTerminal
 * @param {JavaScriptCompiler} compiler
 * @param {string[]} parts
 * @param {number} i
 * @param {string} type
...
function strictLookup(requireTerminal, compiler, parts, i, type) {
  ...
  if (requireTerminal) {
    return [
      ...
      ', ',
      serializeLocation(compiler.source.currentLocation),
      ' )'
    ];
  } 
    return stack;
...
/**
 * Serializes a source location for embedding in compiled output.
 *
 * @param {Record<string, unknown>=} location
 * @returns {string}
 */
