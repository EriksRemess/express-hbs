...
class Compiler {
  ...
  getSignature() {
    this.signature ??= createCompilerSignature(this);
  ...
  accept(node) {
    const nodeType = node != null && Object.hasOwn(node, 'type')
      ? node.type
      : undefined;
    ...
    this.sourceNode.push(node);
    let ret = this[nodeType](node);
    this.sourceNode.pop();
    return ret;
  }
  ...
  // HELPERS
  opcode(name, ...args) {
    const currentNode = this.sourceNode[this.sourceNode.length - 1];
...
export function precompile(input, options) {
  if (
    input == null ||
    typeof input !== 'string' && !isProgramAst(input)
  ) {
    throw new Exception(
...
/**
 * Builds a stable key for the child-program equivalence relation used by equals().
 *
 * @param {{ opcodes: Array<{ opcode: string, args: unknown[] }>, children: Array<{ getSignature: () => string }> }} environment
 * @returns {string}
 */
function createCompilerSignature(environment) {
  const signature = [];
  const opcodes = environment.opcodes;

  for (let i = 0, len = opcodes.length; i < len; i++) {
    const opcode = opcodes[i];
    signature.push(opcode.opcode, '(');
    appendSerializedArgs(signature, opcode.args);
    signature.push(')');
  }
  ...
  for (let i = 0, len = children.length; i < len; i++) {
    signature.push(children[i]?.getSignature() ?? '', '|');
  }
 ...
 * @returns {void}
 */
function appendSerializedArgs(output, args) {
  for (let i = 0, len = args.length; i < len; i++) {
    if (i) {
      output.push(',');
    }
    appendSerializedArg(output, args[i]);
  }
}
 ...
 * @returns {void}
 */
function appendSerializedArg(output, value) {
  if (Array.isArray(value)) {
    output.push('[');
    appendSerializedArgs(output, value);
    output.push(']');
    return;
  }
  ...
  switch (typeof value) {
    case 'string':
      output.push('s:', JSON.stringify(value));
      break;
    case 'number':
      output.push('n:', String(value));
      break;
    case 'boolean':
      output.push('b:', value ? '1' : '0');
      break;
    case 'undefined':
      output.push('u:');
      break;
    case 'object':
      output.push(value === null ? 'l:' : 'o:');
      break;
    default:
