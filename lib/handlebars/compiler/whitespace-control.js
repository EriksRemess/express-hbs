...
const PREV_SIBLING_WHITESPACE = /\r?\n\s*?$/;
const PREV_ROOT_WHITESPACE = /(^|\r?\n)\s*?$/;
const NEXT_SIBLING_WHITESPACE = /^\s*?\r?\n/;
const NEXT_ROOT_WHITESPACE = /^\s*?(\r?\n|$)/;
const RIGHT_STRIP_MULTIPLE = /^\s+/;
const RIGHT_STRIP_SINGLE = /^[ \t]*\r?\n?/;
const LEFT_STRIP_MULTIPLE = /\s+$/;
const LEFT_STRIP_SINGLE = /[ \t]+$/;
const PARTIAL_INDENT = /([ \t]+$)/;
...
function isPrevWhitespace(body, i, isRoot) {
  ...
  if (prev.type === 'ContentStatement') {
    return (sibling || !isRoot ? PREV_SIBLING_WHITESPACE : PREV_ROOT_WHITESPACE).test(
      prev.original
    );
  }
}
...
function isNextWhitespace(body, i, isRoot) {
  ...
  if (next.type === 'ContentStatement') {
    return (sibling || !isRoot ? NEXT_SIBLING_WHITESPACE : NEXT_ROOT_WHITESPACE).test(
      next.original
    );
  }
}
...
function omitRight(body, i, multiple) {
  ...
  current.value = current.value.replace(
    multiple ? RIGHT_STRIP_MULTIPLE : RIGHT_STRIP_SINGLE,
    ''
  );
...
function omitLeft(body, i, multiple) {
  ...
  const original = current.value;
  current.value = current.value.replace(
    multiple ? LEFT_STRIP_MULTIPLE : LEFT_STRIP_SINGLE,
    ''
  );
    ...
    for (let i = 0; i < body.length; i++) {
      ...
      if (doStandalone && inlineStandalone) {
        ...
        if (omitLeft(body, i) && current.type === 'PartialStatement') {
          current.indent = PARTIAL_INDENT.exec(body[i - 1].original)[1];
        }
      }
