...
class SourceNode {
   ...
   */
  add(chunks) {
    this.src += stringifyChunk(chunks, false);
  }
   ...
   */
  prepend(chunks) {
    this.src = stringifyChunk(chunks, false) + this.src;
  }
...
/**
 * Converts generated fragments into source text without allocating nested SourceNode wrappers.
 *
 * @param {unknown} chunk
...
export default class CodeGen {
  ...
  generateList(entries) {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      ret.add(stringifyChunk(entries[i], false));
    }
