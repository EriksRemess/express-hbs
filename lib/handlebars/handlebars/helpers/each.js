...
export default function(instance) {
  instance.registerHelper('each', function(context, options) {
    ...
    let fn = options.fn,
      ...
      data,
      contextPath;
    const blockParamCount = fn.blockParams || 0;
    ...
    function execIteration(field, value, index, last) {
      ...
      if (blockParamCount > 0) {
        const blockParamValues = blockParamCount > 1 ? [value, field] : [value];
        const blockParamPaths = blockParamCount > 1 ? [contextPath + field, null] : [contextPath + field];
        iterationOptions.blockParams = blockParams(blockParamValues, blockParamPaths);
      }
