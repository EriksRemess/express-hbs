...
export function template(templateSpec, env) {
  ...
  function wrapHelperWithLookup(helper) {
    ...
    const cached = helperWrapperCache.get(helper);
    if (cached) {
      return cached;
    }

    const wrapped = wrapHelper(helper, container.lookupProperty);
    helperWrapperCache.set(helper, wrapped);
    return wrapped;
  }
  ...
  function createPartialInvocationOptions(partial, options, hooks, protoAccessControl) {
    ...
    partialOptions.data = options.data ? createFrame(options.data) : {};
    ...
    if (
      ...
      Object.assign(partialOptions, partial._getIsolatedPartialState(options));
    }
  ...
  function invokePartialWrapper(partial, context, options) {
    ...
    if (!options.hash && !options.fn && !options.indent && typeof partial === 'function') {
      const partialOptions = preparePartialExecutionOptions(
        createPartialInvocationOptions(
          partial,
          options,
          this.hooks,
      ...
      return partial(context, partialOptions);
    }
    ...
    if (result != null) {
      if (partialOptions.indent) {
        ...
        for (let i = 0, l = lines.length; i < l; i++) {
          ...
          lines[i] = partialOptions.indent + lines[i];
        }
        result = lines.join('\n');
      }
      return result;
  ...
  const container = {
    ...
    mergeIfNeeded(param, common) {
      ...
      if (param && common && param !== common) {
        obj = Object.assign({}, common, param);
      }
    ...
    if (!options.partial) {
      ...
      if (templateSpec.usePartial || templateSpec.useDecorators) {
        container.decorators = options.decorators
          ? Object.assign({}, env.decorators, options.decorators)
          : env.decorators;
      }
...
function wrapProgram(container, i, fn, ...rest) {
  const [data, declaredBlockParams, blockParams, depths] = rest;
...
function executeDecorators(fn, prog, container, ...rest) {
  const [depths, data, blockParams] = rest;
  if (fn.decorator) {
    const props = {};
    prog = fn.decorator(
      prog,
