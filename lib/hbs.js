...
class ExpressHbs {
  ...
  async _ensureInRestrictLayoutsTo(layoutFile) {
    ...
    const root = await fs.realpath(path.resolve(this.restrictLayoutsTo));
    ...
    while (hasResolvers(result)) {
      ...
      if (pendingKeys.length > 0) {
        ...
        for (let index = 0; index < pendingKeys.length; index += 1) {
          ...
          values[key] = resolvedValues[index];
          keys.push(key);
        }
      }
      ...
      const nextResult = this._replaceValue(result, replacements);
  ...
  async _renderFile(filename, source, options) {
    ...
    const viewsDir = normalizePathOption(baseOptions.settings?.views ?? this.viewsDirOpt, 'views', { allowArray: true });

    if (this.hasAsyncHelpers) {
      renderOptions.resolverCache = createLookupObject();
    }
