 ...
 *   handlebars?: import('handlebars').create,
...
const mergeObject = (target, source) => {
  ...
  for (let index = 0; index < keys.length; index += 1) {
    ...
    const sourceIsObject = sourceValue && Object.prototype.toString.call(sourceValue) === '[object Object]';
    if (sourceIsObject) {
      const targetValue = target[key];
      const targetIsObject = targetValue && Object.prototype.toString.call(targetValue) === '[object Object]';
      target[key] = mergeObject(targetIsObject ? targetValue : {}, sourceValue);
      continue;
    }
...
class ExpressHbs {
  ...
  async _cachePartials(viewsDir) {
    ...
    const partialRoots = Array.isArray(this.partialsDir)
      ? this.partialsDir
      : [this.partialsDir];
    let manifestPartials = '';
    for (let index = 0; index < partialRoots.length; index += 1) {
      if (index > 0) {
        manifestPartials += '\n';
      }
      manifestPartials += partialRoots[index];
    }
    const manifestKey = `${this._options.extname}:${manifestPartials}`;
    ...
    const reads = new Array(manifest.length);
    for (let index = 0; index < manifest.length; index += 1) {
      reads[index] = fs.readFile(manifest[index].fullPath, 'utf8');
    }
  ...
  registerAsyncHelper(name, fn) {
    ...
    this.handlebars.registerHelper(name, function(...helperArgs) {
      ...
      return resolver.resolve(resolverCache, resolveFunc, argsWithoutOptions);
    });
  }
  ...
  _renderTemplate(template, locals) {
    try {
      const hasLocalTemplateOptions = !!locals && Object.hasOwn(locals, '_templateOptions');
      const localTemplateOptions = hasLocalTemplateOptions ? locals._templateOptions : undefined;
      const useLocalTemplateOptions = hasEnumerableKeys(localTemplateOptions);
      let localsClone = locals ?? {};
      ...
      return template(localsClone, templateOptions);
    } catch (err) {
      if (err?.message) {
  ...
  _renderWithLayouts(template, locals, layoutTemplates) {
    ...
    const layoutLocals = locals ? { ...locals } : {};
  ...
  /**
   * Lazily loads `js-beautify` and `.jsbeautifyrc` configuration.
   *
  ...
  async _loadBeautify() {
    if (!this.beautify) {
      ...
      this.beautify = beautifyModule.default ? beautifyModule.default.html : beautifyModule.html;

      const rcPath = path.join(process.cwd(), '.jsbeautifyrc');
      try {
        const rcContent = await fs.readFile(rcPath, 'utf8');
        this.beautifyrc = JSON.parse(rcContent);
      } catch (err) {
        if (err.code !== 'ENOENT') {
          throw err;
        }
  ...
  async _resolveLayoutTemplates(filename, templateSource, options, viewsDir) {
    ...
    if ('layout' in options && !options.layout) {
      return null;
    }
    ...
    if ('layout' in options && options.layout) {
      const layoutFile = this.layoutPath(filename, options.layout, viewsDir);
  ...
  async _resolveAsyncHtml(resolverCache, html) {
    ...
    while (resolver.hasResolvers(result)) {
      ...
      for (let index = 0; index < keys.length; index += 1) {
        ...
        if (!resolver.hasResolvers(value)) {
          continue;
        }
  ...
  async _renderFile(filename, source, options) {
    ...
    renderOptions.blockCache = {};
    const viewsDir = renderOptions.settings?.views ?? this.viewsDirOpt;

    if (this.hasAsyncHelpers) {
      renderOptions.resolverCache = Object.create(null);
    }

    await this._loadDefaultLayout(renderOptions.cache, viewsDir);

    if (this.partialsDir && (!renderOptions.cache || !this.isPartialCachingComplete)) {
      await this._cachePartials(viewsDir);
    }

    const info = await this._getSourceTemplate(filename, source, renderOptions.cache, viewsDir);
    const layoutTemplates = await this._resolveLayoutTemplates(filename, info.source, renderOptions, viewsDir);

    let renderedHtml = this._renderWithLayouts(info.template, renderOptions, layoutTemplates);

    if (this._options.beautify) {
      await this._loadBeautify();
      renderedHtml = this.beautify(renderedHtml, this.beautifyrc);
    }
  ...
  ___express(filename, sourceOrOptions, optionsOrCb, maybeCb) {
    ...
    void (async () => {
      try {
        const html = await this._renderFile(filename, source, options);
        cb(null, html);
      } catch (err) {
        cb(err, null);
      }
