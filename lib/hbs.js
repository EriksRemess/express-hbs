...
const pathOptionCacheKey = (value) => Array.isArray(value) ? value.join('\n') : value ?? '';
const layoutCacheKey = (filename, allowedRoot) => `${filename}\n${allowedRoot ? path.resolve(allowedRoot) : ''}`;
  ...
  if (cache instanceof Map) {
    ...
    if (Array.isArray(pendingEntries) && pendingEntries.length > 0) {
      ...
      for (let index = 0; index < queuedEntries.length; index += 1) {
        const [key, value] = queuedEntries[index];
 ...
 */
const normalizePathOption = (value, name, options = {}) => {
  const {
    allowArray = false,
    allowFalse = false
  } = options;
 ...
 * @returns {boolean}
 */
function isUnsupportedGlobWithFileTypesError(error) {
  return error instanceof TypeError
    && typeof error.message === 'string'
    && error.message.includes('withFileTypes');
}
 ...
 */
async function collectFilesWithReaddir(currentDir, files) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await collectFilesWithReaddir(entryPath, files);
      continue;
    }
...
async function listFilesRecursive(rootDir) {
  ...
  if (typeof fs.glob === 'function') {
    try {
      ...
      if (!isUnsupportedGlobWithFileTypesError(error)) {
        throw error;
      }
    }
  }

  await collectFilesWithReaddir(rootDir, files);
  return files;
}
...
class ExpressHbs {
  ...
  layoutPath(filename, layout, viewsDir) {
    ...
    const cacheKey = `${filename}\n${layout}\n${pathOptionCacheKey(viewsDir)}\n${pathOptionCacheKey(this.layoutsDir)}`;
    if (this.layoutPathCache.has(cacheKey)) {
      return this.layoutPathCache.get(cacheKey);
    }

    if (layout.startsWith('.')) {
      const resolved = path.resolve(this._dirname(filename), layout);
      this.layoutPathCache.set(cacheKey, resolved);
      return resolved;
    }
    ...
    if (this.layoutsDir) {
      if (Array.isArray(this.layoutsDir)) {
        if (this.layoutsDir.length === 0) {
          this.layoutPathCache.set(cacheKey, undefined);
          return;
        }
        const resolved = path.resolve(this.layoutsDir[0], layout);
        this.layoutPathCache.set(cacheKey, resolved);
        return resolved;
      }
      const resolved = path.resolve(this.layoutsDir, layout);
      this.layoutPathCache.set(cacheKey, resolved);
      return resolved;
    }
    ...
    if (Array.isArray(viewsDir)) {
      ...
      const resolved = path.resolve(viewsDir[0], layout);
      this.layoutPathCache.set(cacheKey, resolved);
      return resolved;
    }
    const resolved = path.resolve(viewsDir, layout);
    this.layoutPathCache.set(cacheKey, resolved);
    return resolved;
  }
  ...
  async _ensureInRestrictLayoutsTo(layoutFile) {
    ...
    await this._ensureLayoutWithinRoot(layoutFile, this.restrictLayoutsTo);
  }
   ...
   */
  async _cacheLayout(layoutFile, useCache, viewsDir, allowedRoot = this.restrictLayoutsTo, seenLayouts = new Set()) {
    ...
    try {
      await this._ensureLayoutWithinRoot(targetLayoutFile, allowedRoot);

      if (!useCache) {
        const { compiled, parentLayoutFile } = await this._getUncachedLayoutInfo(targetLayoutFile, viewsDir);

        let layouts = [];
        if (parentLayoutFile) {
          layouts = await this._cacheLayout(parentLayoutFile, useCache, viewsDir, allowedRoot, seenLayouts);
        }
  ...
  async _cachePartials(viewsDir) {
    ...
    const stats = await Promise.all(manifest.map(({ fullPath }) => fs.stat(fullPath)));
    ...
    const sources = await Promise.all(changedEntries.map(({ entry }) => fs.readFile(entry.fullPath, 'utf8')));
  ...
  registerAsyncHelper(name, fn) {
    ...
    this.handlebars.registerHelper(name, function(...helperArgs) {
      ...
      const resolveFunc = (args, cb) => {
        if (includeOptions) {
          return fn.call(this, ...args, options, cb);
        }
  ...
  _renderTemplate(template, locals) {
    ...
    try {
      ...
      if (useGlobalTemplateOptions || useLocalTemplateOptions) {
        templateOptions = useGlobalTemplateOptions ? mergeObject({}, this._options.templateOptions) : {};
   ...
   */
  async _getUncachedLayoutInfo(filename, viewsDir) {
    ...
    if (
      ...
      const cachedCompiled = cached.compiledByViews.get(viewsKey);
      if (cachedCompiled) {
        return {
      ...
      const compiled = this.compile(cached.source, filename, viewsDir);
      cached.compiledByViews.set(viewsKey, compiled);
      return {
        compiled,
        parentLayoutFile: cached.parentLayoutFile
      };
   ...
   */
  async _getUncachedTemplateInfo(filename, viewsDir) {
    const stat = await fs.stat(filename);
    const viewsKey = pathOptionCacheKey(viewsDir);
    ...
    if (
      ...
      const cachedInfo = cached.infoByViews.get(viewsKey);
      if (cachedInfo) {
        return cachedInfo;
      }
      ...
      cached.infoByViews.set(viewsKey, info);
      return info;
    }

    const templateSource = await fs.readFile(filename, 'utf8');
  ...
  async _getSourceTemplate(filename, source, useCache, viewsDir) {
    ...
    if (typeof source !== 'string' && !useCache) {
      return this._getUncachedTemplateInfo(filename, viewsDir);
    }
   ...
   * @returns {Promise<Function[] | null>}
   */
  async _resolveLayoutTemplates(filename, templateInfo, options, viewsDir) {
    ...
    if (declaredLayoutFile) {
      ...
      declaredLayoutTemplates = await this._cacheLayout(declaredLayoutFile, options.cache, viewsDir, allowedRoot);
    }
    ...
    if (hasLayoutOption && optionLayout) {
      ...
      return this._cacheLayout(layoutFile, options.cache, viewsDir, allowedRoot);
    }
    ...
    if (this._options.defaultLayout) {
      return this._loadDefaultLayout(options.cache, viewsDir);
    }
    ...
    while (true) {
      ...
      if (pendingKeys.length > 0) {
        ...
        for (let index = 0; index < pendingKeys.length; index += 1) {
          ...
          const escapedId = escapeExpression(key);
          values[key] = resolvedValue;
          replacementEscapedIds[key] = escapedId;
      ...
      if (nestedValueKeys.length > 0) {
        const remainingNestedValueKeys = [];

        for (let index = 0; index < nestedValueKeys.length; index += 1) {
          const key = nestedValueKeys[index];
          const value = values[key];
          ...
          if (hasResolvers(values[key])) {
            remainingNestedValueKeys.push(key);
          }
        }

        nestedValueKeys = remainingNestedValueKeys;
      }

      if (changedNestedKeys.length > 0) {
        this._updateAsyncReplacements(values, replacements, changedNestedKeys, replacementEscapedIds);
      }

      const nextResult = this._replaceValue(result, replacements, replacementCount);
  ...
  async _renderFile(filename, source, options) {
    ...
    const viewsDir = this._normalizeViewsDir(baseOptions.settings?.views ?? this.viewsDirOpt);
    ...
    const layoutTemplates = await this._resolveLayoutTemplates(filename, info, baseOptions, viewsDir);
    ...
    if (!hasResolvers(renderedHtml)) {
      await this._drainAsyncValues(renderOptions.resolverCache);
      return renderedHtml;
    }
