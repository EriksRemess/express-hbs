...
const asyncPlaceholderPattern = /__aSyNcId__(?:<_|&lt;_)[A-Za-z_]{8}__/g;
...
class ExpressHbs {
  ...
  async _ensureInRestrictLayoutsTo(layoutFile) {
    ...
    const root = this.restrictLayoutsRootRealpath ?? await fs.realpath(path.resolve(this.restrictLayoutsTo));
    this.restrictLayoutsRootRealpath = root;
    const resolved = await fs.realpath(path.resolve(layoutFile));
  ...
  async _cacheLayout(layoutFile, useCache, viewsDir) {
    ...
    await this._ensureInRestrictLayoutsTo(targetLayoutFile);
  ...
  async _resolveAsyncHtml(resolverCache, html) {
    ...
    while (hasResolvers(result)) {
      ...
      if (pendingKeys.length > 0) {
        ...
        resolvedEntryCount += pendingKeys.length;
      }

      if (resolvedEntryCount === 0) {
        throw new Error('Encountered unresolved async placeholder without a matching resolver entry.');
      }
