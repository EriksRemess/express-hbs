...
const ID_COUNTER_HEX_LENGTH = 16;
...
const ID_SUFFIX = '__';
const RESOLVER_ID_PATTERN = /__aSyNcId__(?:<_|&lt;_)[a-f0-9]{32}__/;
const resolverIdPrefixKey = Symbol('resolverIdPrefix');
const resolverIdCounterKey = Symbol('resolverIdCounter');
export const resolverPendingEntriesKey = Symbol('resolverPendingEntries');
...
function generateId(cache) {
  cache[resolverIdPrefixKey] ??= randomBytes(ID_PREFIX_LENGTH).toString('hex');
  cache[resolverIdCounterKey] ??= 0;

  const counter = cache[resolverIdCounterKey];
  cache[resolverIdCounterKey] = counter + 1;

  const counterHex = counter.toString(16);
  if (counterHex.length <= ID_COUNTER_HEX_LENGTH) {
    return cache[resolverIdPrefixKey] + counterHex.padStart(ID_COUNTER_HEX_LENGTH, '0');
  }

  return randomBytes(ID_LENGTH).toString('hex');
}
...
export function resolve(cache, fn, context) {
  ...
  const id = `${ID_PREFIX}${ID_ESCAPED_STRING}${generateId(cache)}${ID_SUFFIX}`;
  const pending = new Promise((resolvePromise, rejectPromise) => {
    try {
      const result = fn(context, resolvePromise);
      if (result && typeof result.then === 'function') {
        result.then(resolvePromise, rejectPromise);
      }
    } catch (error) {
      rejectPromise(error);
    }
  });
  // Observe failures even if rendering throws before it can drain the cache.
  // Keep the original promise so done() still reports the helper's rejection.
  void pending.catch(() => {});
